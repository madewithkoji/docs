...
To ensure a template can be displayed correctly in the Koji feed, you must implement the `FeedSdk` and enable the `FeedEvents` entitlement.
...
To use this context:
** *Define an About page experience* – Enable the `AboutContext` entitlement, and define a view intended for the About page.
+
...
To use this context:
** *Define an admin experience* – Enable the `AdminContext` entitlement, and define a view intended for the creator of the Koji.
For example, enable the creator to respond to questions from users.
+
NOTE: When the `AdminContext` entitlement is enabled, the creator can access the admin view by going to *Koji > Admin Portal*.
...
** *Define a sticker experience* – Enable the `StickerContext` entitlement, and define a view that loads only when the Koji is viewed as a sticker (with the URL parameter `context=sticker`).

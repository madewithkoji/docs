...
Receieves the following property as input:
...
Receieves the following property as input:
** `newValue` – _Any_, the value set within the modal once it has been resolved.
...
Receieves the following property as input:
** `url`  – _String_, the url of the uploaded file.

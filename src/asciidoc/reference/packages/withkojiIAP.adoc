...
<9> `transactionIds` – Object containing the unique identifiers for the associated transactions.
To link to the transaction record on the Koji platform, use the format `\https://withkoji.com/payments/transactions/TXN_ID`.

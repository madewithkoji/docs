...
<7> <<_iapproduct>> object that represents the purchased product.
<8> Object that represents the transaction information, if available.
...
----
<1> Unique identifier for this version of the product.
<2> Name of the Koji template from which the product was purchased.
<3> Koji user name of the template publisher.
<4> Purchase price of the product.
Defined in the  <<#_iap,entitlements file>> of the template.
<5> Indicator of whether a purchase price is defined for the product.
Defined in the entitlements file of the template.
<6> Indicator of whether a product can be purchased more than once.
Defined in the entitlements file of the template.
<7> Description displayed when the user was prompted to purchase the product.
Defined in the entitlements file of the template.
<8> Identifier of the purchased product.
Defined in the entitlements file of the template.
<9> Date the product was registered or updated, which happens when the template is published.
<10> Indicator of whether the product is still available for purchase.
<11> Type of user information collected for order fulfillment.
Defined in the entitlements file of the template.
<12> Total number of times the product can be sold (inventory threshold).
Defined in the entitlements file of the template.
<13> Remaining number of times the product can be sold.
Calculated based on the total inventory defined in the entitlements file, less the number of purchases.
<14> Object that represents the template publisher ("seller").
<15> Array of <<_iapreceipt>> objects representing purchases of the product.

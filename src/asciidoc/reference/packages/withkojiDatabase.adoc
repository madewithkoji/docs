...
NOTE: The `searchAttribute` must have a value set for all entries in your database.
If some entries have no value for that key, nothing will be returned.
...
NOTE: Only updates specified values, anything not present in `data` will be left unchanged.

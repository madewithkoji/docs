...
WARNING: The <<#.get, get()>> function will work before or after <<#.ready, ready()>> has been called.
However, all the listeners should be set before calling `ready()`.
...
The secret VCC allows setting of sensitive data such as premium images in a VCC without the value being visible when projects get remixed.
...
_String_, value of the secret VCC.

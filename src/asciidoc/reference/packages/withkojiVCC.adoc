...
In order for your TypeScript projects to properly work with Koji VCCs, you will need to add the following line to the `scripts` section of your project's `package.json`:

[source, JSON]
"prebuild": "koji-vcc preinstall-ts"

This causes your app to build the VCC config files prior to compiling your TypeScript code when publishing, just like when koji-vcc sets a watch on your config files during development.
This is necessary to prevent errors by TypeScript during compile time.
...
* `path` – Array of _Strings_, keys pointing to the desired VCC value or either `metadata` or `serviceMap`

==== Returns

_Any_, value of the <<vcc-overview#_vcc_types,VCC type>> at the specified path.
For the predefined environment values the following will be returned:
...
** `editorAttributes` – _Object_, describes the current editor. Contains the following attributes:
*** `type` – _String_, describes the current editor type, either `full` or `instant`.
...
This middleware is required to manage the environment variables to scope them for instant remixes of the original template. In particular when using <<withkoji-koji-iap-package#>> or <<withkoji-database-package#>> you must call this middleware before instantiating those packages to ensure the correct environment is used by those packages.
...
When you are confident that your app is able to function well within a feed, please contact us for review and authorization.

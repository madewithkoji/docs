...
<2> `AboutContext` – (Optional) Displays a distinct experience when the template is remixed (About page).
See <<_context>>.
...
<3> `AdminContext` – (Optional) Enables a distinct experience for the creator of a Koji (when viewed with the `context=admin` URL parameter).
See <<_context>>.
...
+
NOTE: When the `AdminContext` entitlement is enabled, the creator can access the admin view by using the *Koji button* in the top right, and then going to *Koji Settings > Open Admin Page*.

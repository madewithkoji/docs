...
So, in order to create a remixable template, we’ll need a way to structure our remix-specific data, collect those values from the user, and then store them in Koji’s database.
Fortunately, the @withkoji/core package will do a lot of the heavy lifting for us.

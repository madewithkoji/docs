...
[NOTE]
The Koji editor is recommended for this tutorial, but you're free to <<use-git#, clone the newly created project to your local environment>> if you prefer.
...
The game also need sounds to play when the player collects or misses a falling object.
...
Update `koji.json` to add the URL paths for the images and sounds as follows.
...
To load images, use the `addImage()` function from the template.
...
Then, you customized the starting values for game settings and loaded the image and sound assets into the game.

In the <<game-creating-game-objects#,next section>>, you'll add the `Player` and `Collectible` classes.

...
:page-description: Making the price customizable during remix for a game with in-app purchases.
:figure-caption!:
...
For the price input field, this tutorial uses a pre-made component that is included in the @anderjason/koji-react-frontend-tools package.
The component works well for this game because it already handles the proper formatting and validation, so you don't have to worry about the user entering invalid characters, incorrect price formats, or other invalid data.
And, the package is already installed in the base game template.

This tutorial also uses the @anderjason/money package, which you need to install in the frontend of your game.
...
Finally, you need to update the changed `price` in the remix data.
...
Your remix menu should now look like this example.

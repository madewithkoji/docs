...
When developing such a template, the recommended way to upload files is to use the `generateSignedUploadRequest` method from the <<withkoji-database-package#.generateSignedUploadRequest, @withkoji/database>> package.
...
include::{includespath}/prereqs.adoc[tags=webdev;expressjs;remixproc;customvcc;backend;kojidb]

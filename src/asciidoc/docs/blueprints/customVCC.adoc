= Custom VCC Template
:page-slug: custom-vcc-blueprint
...
* Understanding of third party API usage beneficial.
...
Install `@withkoji/custom-vcc-sdk` to enable custom VCC functionality such as loading and saving the VCC value from Koji and install the development dependency @babel/plugin-proposal-class-properties to add support for some extra syntax to be compiled by babel.
...
Add the babel plugin to the babel runtime configuration file to ensure it can use it when compiling

[source,json]
----
include::../_includes/vcc-blueprint-babelrc.json[]
----

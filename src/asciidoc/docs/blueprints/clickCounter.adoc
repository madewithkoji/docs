...
:page-slug: click-counter-blueprint

Follow this blueprint to build a template that counts clicks from multiple users in real-time.
You will implement a database to record the users and clicks and real-time dispatch to provide users with an immediate, multi-user experience.
Then, you will publish the template on Koji.

image::click-counter.png[Make a click counter template on Koji,role="img-overview"]
...
This template relies on a backend service to store the total number of clicks from different users and sessions.
...
Add a listener to monitor the `new_tap` event, which fires whenever a user clicks the icon.
...
Submit click counts to the backend for long-term storage in the database.
...
When a user clicks the icon, increment the counter and emit the `new_tap` event with dispatch to allow other clients to respond to the click.
...
.React
----
include::../_includes/clickCounterReact.js[tags=importPackages]
----

[source,javascript]
.Vanilla JS
----
include::../_includes/clickCounterVanilla.js[tags=importPackages]
----
...
[source,javascript]
----
include::../_includes/clickCounterReact.js[tags=feedSdk]
----
...
[source,javascript]
----
include::../_includes/clickCounterReact.js[tags=initDataHandler]
----

Track the remix state and the number of clicks.

[source,javascript]
.React
----
include::../_includes/clickCounterReact.js[tags=trackState,indent=0]
----

[source,javascript]
.Vanilla JS
----
include::../_includes/clickCounterVanilla.js[tags=trackState,indent=0]
----

Set the callback for `DataHandler` to update the total clicks.
Set instant remixing to ready to allow remixes and use FeedSdk so your application can be displayed in the Koji feed.

[source,javascript]
.React
----
include::../_includes/clickCounterReact.js[tags=callDataHandler,indent=0]
----

[source,javascript]
.Vanilla JS
----
include::../_includes/clickCounterVanilla.js[tags=callDataHandler,indent=0]
----

Update the rendered output to display the current click count, and implement click events for instant remixing and for counting clicks.
Use the `instantRemix.onPresentControl` callback to display the VCC controls when users are remixing your application.

[source,javascript]
.React
----
include::../_includes/clickCounterReact.js[tags=renderTemplate,indent=0]
----

[source,javascript]
.Vanilla JS
----
include::../_includes/clickCounterVanilla.js[tags=renderTemplate,indent=0]
----
...
.React
----
include::../_includes/clickCounterReact.js[tags=styles;stylesOnly,indent=0]
----
<1> Remove the `pointer-events: none;` property definition.
...
* Multi-user experience – On the *Remote* tab, use *Open in new tab* to open the application in multiple tabs and ensure the click counter remains synchronized.
...
.React
https://withkoji.com/~JamesHole/database-and-dispatch-template[Blueprint: Click Counter (React)]

.Vanilla JS
https://withkoji.com/~JamesHole/database-and-dispatch-template-vanilla-js[Blueprint: Click Counter (Vanilla JS)]
...
.React
----
include::../_includes/clickCounterReact.js[tags=**;!stylesOnly]
----

[source,javascript]
.Vanilla JS
----
include::../_includes/clickCounterVanilla.js[tags=**]
----

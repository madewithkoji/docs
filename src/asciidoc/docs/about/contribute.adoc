...
:page-description: A guide for contributing to the Koji for Developers site.
...
Understanding the file structure for the documentation will also be helpful for making modifications.
Whilst all the documentation is structured as per the site under `src/asciidoc/` there are two other file structures to understand that will help you build content:

nav.json::
	The navigation of the site is handled in a single json file `src/nav.json`. This structure uses the `name` and `slug` attributes to define where the pages sit in the navigation both for the sidebar and the overall url structure.
_includes/::

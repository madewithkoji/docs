...
Page slug::
	Corresponds to the page URL and is used for linking between different pages. +
...
TIP: If the page contains block images, you don't need to set the page banner attribute. The first block image on the page is used, instead.
...
To sync multiple sets of tabs, use the `scope` attribute and matching tab titles. Then, selecting one tab on the page also selects any tabs with the same scope and the same title.

...
If the project has a backend, you also need the values of the *`*KOJI_PROJECT_ID` and `KOJI_PROJECT_TOKEN` environment variables.
...
+
NOTE: Keep the dialog open for the next two steps.

=== Obtaining Koji environment variables

[NOTE]
If your project does not have a backend, skip this section.

If your project has a backend, you need the values of the `KOJI_PROJECT_ID` and `KOJI_PROJECT_TOKEN` environment variables to access the leaderboard database.

. If the Clone dialog is not already open, return to the browser tab for *Project Details*, and click *Clone*. The dialog opens.
. Click the first line of text under *Environment Variables* to highlight it.
. Copy the highlighted line to the clipboard.
. Paste the line somewhere to save it for later.
. Do the same with the second line of text.

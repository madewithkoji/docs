...
* Learn about the tools required for working locally and how to install them.
* Learn where to find the URL, username, and password that you'll need to clone your project.
* Learn how to clone the Git repository for your project onto your local machine.
...
This tutorial uses the *Falling Objects* template as an example because it illustrates a completed game with code that runs both on the backend and on the frontend.
...
. Go to the https://withkoji.com/~Svarog1389/rxkd[Falling Objects] project.
. {blank}
+

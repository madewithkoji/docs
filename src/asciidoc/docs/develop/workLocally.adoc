...
===== Additional Windows steps for launching the frontend
...
+
Alternatively, in case `npm` fails to read the values from the `.env` file, you can set the environment variables manually in the terminal before starting the server:
+
Linux:
+
[source,bash]
...
+
With `KOJI_PROJECT_ID` and `KOJI_PROJECT_TOKEN` being the values you previously fetched.
+
Another alternative would be to use the https://www.npmjs.com/package/dotenv[dotenv]  package to read the values from the `.env` file.
+
...
===== Additional Windows steps for launching the backend
...
```
- Remove any previous duplicate scripts

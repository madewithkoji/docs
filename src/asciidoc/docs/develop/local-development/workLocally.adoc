...
You can manually launch the servers, using separate terminal windows for each, or you can set up a launch configuration file, if you're working in VS Code.
...
. Replace the `KOJI_PROJECT_ID` and `KOJI_PROJECT_TOKEN` values with the environment variable values for your project, and save the file.

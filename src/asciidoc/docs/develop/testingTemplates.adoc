...
* You can open it in  *Live player*.
This opens another browser window where the Koji is displayed in an iframe, just as the published Koji will display.
...
A Koji can display different template experiences depending on the context in which it is running.
...
A custom context cannot be viewed in *Live preview*.
You can view it in *Live player* by appending `?context=test` to the URL (see <<_previewing_the_koji>>).
...
. Log into your Koji account and click on your profile icon to open the main menu.
. Click on *Wallet*.
...
If you have videos or audio files in your Koji that play automatically, you might want them to play only when they are visible to the user.

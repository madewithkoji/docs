...
You can display different template experiences depending on the context in which the template is running. For more on contexts, see <<withkoji-vcc-package#_context>>.

Koji provides a number of standard contexts.
If a standard context is enabled in your template, a button for it appears in the *Live preview* pane.
Clicking the button for a context displays a preview of it.

The following is a list of Koji's standard contexts.


* *Default* – Displays what a template looks like when running. This context is always enabled.
...
* *Thumbnail* – Displays what a template looks like when shared on social media. This context is always enabled.
...
Two more contexts are available if you implement <<withkoji-koji-iap-package#_iap,in-app purchases>>.
...
[NOTE]
If you create a custom context for your template, you can preview it in *Live player*. See <<_remote_previews>>.

...
The Koji debugger allows you to follow the communication happening between the Koji platform and the template.
The debugger is only available after you publish your Koji for the first time.

To display the debugger, open your template in the Koji editor.
Then click *Open debugger*, which is in the left pane under *Open published app*.
...
Selecting the *Default* context shows you a preview of the running template.
Events associated with the *Default* context are displayed in an event pane.
See <<_viewing_events>>.

In the *Default* context, you can see what happens when the template asks the platform for things like <<withkoji-koji-iap-package#, in-app purchases>> or <<withkoji-koji-auth-sdk#, auth grants>>.

==== Remix context

Selecting the *Remix* context displays a special remixing page, if you defined one in your template.
Otherwise, it displays the editable elements of your template and allows you to change them.
A separate pane displays the *VCC Values* as they appear in the code for the Koji.
Events associated with the *Remix* context are displayed in an event pane.
See <<_viewing_events>>.

In *Remix*, you can see the way VCC values change in response to user interactions, as well as how the platform sets and accepts new values from the template.

==== *Thumbnail*

Selecting the *Thumbnail* context displays previews of what your Koji will look like if you share it on social media
See <<rich-preview-image#>>.

The preview shows the thumbnail in both square and rectangular format.

==== Admin

If the *Admin* context is enabled, the admin page is displayed.
Events associated with the *Admin* context are displayed in an events pane.
See <<_viewing_events>>.

==== About

If the *About* context is enabled, a page is displayed that appears when the user clicks *Remix this Koji*.
Events associated with the *About* context are displayed in an events pane.
See <<_viewing_events>>.

==== Sticker

If another template is embedded in your template, a page for the embedded template is displayed.
Events associated with the *Sticker* context are displayed in an events pane.
See <<_viewing_events>>.

==== *Dynamic Receipt*

If a dynamic receipt is enabled, a page is displayed for editing it.
See <<withkoji-koji-iap-package#, in-app purchases>>.

=== Viewing events

Each context generates events that are sent to the Koji platform, and the Koji platform generates events that are sent to the various contexts.
In most cases, selecting one of the contexts in the debugger displays an events pane which shows the events sent to and from that context.
An events pane allows you to see in real time the events that are generated as you interact with a context.
Events generated by the template are listed on the right side of the events pane.
Events generated by the Koji platform are listed on the left side.
...
If you open a template and select the *Remix* context, here's an example of some remix events that might be displayed in the events pane:
...
You can view and modify environment info for your template, as well as settings for implemented features.
If you try to view settings for a feature that is not implemented, a message will be displayed saying that no data was found for the feature.

The *Info and Features pane* includes the following panels:

==== *Environment info*

The *Environment info* panel displays your templates's *App Id*, frame URL, and *VCC editor source*.
You can edit the frame URL and the VCC editor values.

==== *Database*

If your template uses the Koji database, the database tables will be displayed in a panel.
Table records can be viewed and deleted.

==== *In-app purchase receipts*

If *in-app purchases* is implemented in your template, the *in-app purchase receipts* panel allows you to view and delete receipts.

==== *Auth grants*

If your template uses auth grants, the *Auth grants* panel will display them.

...
Selecting *Default* shows you a preview of the running template.
...
Selecting *Remix* displays a special remixing page, if you defined one in your template.
Otherwise, it displays the editable elements of your template and allows you to change them.
A separate pane displays the *VCC Values* as they appear in the code for the template.
Events associated with the *Remix* context are displayed in an event pane.
See <<_viewing_events>>.

*Remix* gives you a dynamic view of remix events, for a better understanding of how VCC data is changing over time in response to user actions.
...
If the admin context is enabled, selecting *Admin* displays the admin page.
Events associated with the admin context are displayed in an events pane.
See <<_viewing_events>>.

==== About

If the about context is enabled, clicking *About* displays the page that appears when the user clicks *Remix this Koji*.
Events associated with the about context are displayed in an events pane.
See <<_viewing_events>>.

==== Sticker

The *Sticker* context shows you how your template behaves when it's embedded inside another template.
...
If you click on a feature that is not implemented, a message will be displayed saying that no data was found for the feature.
...
You can click *Modify* to edit the frame URL.

The *Environment info* panel also allows you to edit the *VCC editor source*.
...
Suppose you're working on a new version of the template that uses a profile video instead of a profile image.
You can override the live data by changing `"type": "image"` to `"type": "video"`, and by changing `"name": "Profile Image",` to `"name": "Profile Video",`.
Now, when working inside the debugger, the remixer will show a video VCC instead of an image VCC, and the prompt for changing the video will be "Profile Video".
...
If your template has any auth grants, *Auth grants* displays them.
It also allows you to easily delete auth grants.

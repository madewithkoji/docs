...
Selecting the *Default* context shows you a preview of the running template. Events associated with the *Default* context are displayed in an event pane.
See <<_viewing_events>>.
...
Selecting the *Thumbnail* context displays previews of what your Koji will look like if you share it on social media. See <<rich-preview-image#>>.
...
Each context generates events that are sent to the Koji platform.
The Koji platform also generated events that are sent to the various contexts.
In most cases, selecting one of the contexts displays an events pane in which the events sent to and from that context are displayed.
The events pane allows you to see in real time what events are generated as you interact with a context.
...
For example, if you open a template in the debugger and click on the *Remix* context, you might see the following events displayed in the events pane:

...
. Go to the temp folder, and copy the package.json file to the frontend folder.
It will replace the package.json file currently in that folder.
+
[source,bash]
----
cd temp
...
rm -rf temp
----
. Run the npm install command to update the npm packages, then run the start command, to refresh the preview and see the Mortgage Overpayment Calculator.
[source,bash]
----
npm install
npm start

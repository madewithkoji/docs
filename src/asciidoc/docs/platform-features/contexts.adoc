...
////
=== Alternative implementation using subscribe

To determine when the user switches to and from the *Remix* context, you can use `Koji.playerState.subscribe`.
...
if (isRemixing) {  // <3>
  return (
    <div className="App">
      <header className="App-header" style={{ backgroundColor }}>
        <button onClick={captureColor}>{'Change the background color'}</button>  // <3>
      </header>
    </div>
  )
}

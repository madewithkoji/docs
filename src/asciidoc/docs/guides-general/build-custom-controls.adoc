...
. <<_create_a_new_project,Create a new project and install the @withkoji/core package.>>
...
Any app that calls your custom control must provide its `koji-vccs.com` subdomain or its URL.

To get your own subdomain of `koji-vccs.com`,
include::{includespath}/steps-domains.adoc[tag=get-subdomain]
. Select the domain `koji-vccs.com`.
. Enter a custom subdomain name.
...
* Using the custom control's URL:
+
[source,javascript]
----
...
Learn more: <<core-frontend-ui-capture#>>

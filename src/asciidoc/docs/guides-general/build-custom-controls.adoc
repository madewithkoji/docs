...
NOTE: The <<withkoji-koji-core#,@withkoji/core>> and <<withkoji-custom-vcc-sdk#,@withkoji/custom-vcc-sdk>> packages are already included in the custom control scaffold and do not need to be reinstalled. However, if you fork from a different scaffold or app, you must install both packages in your app's front end.

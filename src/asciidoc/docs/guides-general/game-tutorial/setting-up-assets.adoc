...
The https://withkoji.com/apps/78ddd0e8-de2d-498e-bd88-255166d54849[Game Dev Scaffold] is a great starting point for this app because it was created specifically for these types of hyper-casual games.
...
. Open the https://withkoji.com/apps/78ddd0e8-de2d-498e-bd88-255166d54849[game scaffold] listing.
. Click btn:[Fork].
+
...
Because there's a considerable difference between mobile and desktop display sizes, you can add a line to the `isMobile()` function to set the `playerSize` according to the device, as follows.
(Actually, this line should already be present in the scaffold, so just double-check that it's there.)
[source,javascript]
----

...
If your Koji's code uses any of the deprecated packages, follow this guide to migrate it to the @withkoji/core package.
For example, you might have implemented features with the deprecated packages or forked a template that uses them.
To determine whether this guide applies to your code base, check the `package.json` file for the deprecated packages, such as `@withkoji/vcc`.
Also, check whether your project contains a `.koji` directory.
If you find a deprecated package or a `.koji` directory, your code base likely needs to be updated.
...
For more information about these settings, see <<app-versions#_managing_instant_remix_versions, Managing remix versions>>.
...
A Koji can provide different experiences for different modes, called *contexts*.
For example, you might provide a different experience during a configuration (remix) than when the template is running.

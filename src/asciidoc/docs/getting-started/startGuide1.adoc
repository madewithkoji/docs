...
----
--
. Create a new instance of the `InstantRemixing` class to use in the template.
+
[.tabs,scope="lang"]
--
.React
...
----
NOTE: In the React component, add the `componentDidMount` method.
...
----
NOTE: Above the `render` function definition, create a new instance of the  class and alert Koji that instant remixing is ready to use.
...
----
NOTE: In the Angular component, add the `ngOnInit` method.
...
----
NOTE: In the Svelte component, add the `onMount` method.
...
----
NOTE: Create a state to handle updates to the title value.
...
----
NOTE: Create a variable to handle updates to the title value.
Also, add an `id` attribute to access the element more easily in the next step.
...
----
NOTE: Add a property to handle updates to the title value.
...
----
NOTE: Add the listener in the `componentDidMount` method.
...
----
NOTE: Add the listener below the `render` function call.
...
----
NOTE: Add the listener in the `ngOnInit` method.
...
----
NOTE: Add the listener in the `onMount` method.
...
----
NOTE: In the `App.js` file, use a dynamic `className`.
...
----
NOTE: Add implementations of `startRemix` and `stopRemix` that modify the class name of the `h1` element, for simplicity in this tutorial.
...
----
NOTE: In the `app.component.html` file, use a dynamic `class`.
...
this.feed.load();
----
NOTE: Add this code at the end of the `componentDidMount` method.
...
feed.load();
----
NOTE: Add this code below the `render` function call.
...
this.feed.load();
----
NOTE: Add this code at the end of the `ngOnInit` method.
...
feed.load();
----
NOTE: Add this code at the end of the `onMount` method.

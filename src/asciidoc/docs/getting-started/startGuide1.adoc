...
. To run the watcher concurrently with your development server, reconfigure the `package.json` file, if needed.
+
[.tabs,scope="lang"]
--
.React
...
=====
Not needed.
This scaffold already uses a `prestart` command in the `package.json` file, as follows.
...
"scripts": {
  ...
  "start": "cross-env NODE_ENV=development ng serve --host 0.0.0.0 --port 4200 --disableHostCheck=true",
  "build": "cross-env NODE_ENV=production ng build --prod --deleteOutputPath=false",
  ...
}
----
=====
...
[source,JavaScript]
----
include::../_includes/startGuide1Angular.js[]
----
====

...
As a developer, you can build Koji templates for any type of web application, including visual biographies, memes, games, premium content sales, and utilities.
...
Or, you could find a Koji that implements some of your desired functionality, such as a backend database.
...
To provide a UI for customizing values, you can use the built-in <<vcc-overview#, Visual Customization Controls>> (VCCs) on the Koji platform for standard elements, such as images, text, and sounds.
...
It is recommended that all Koji templates implement support for the Koji feed, using the `FeedSdk` in the @withkoji/vcc package.
